...
import com.laytonsmith.PureUtilities.ClassLoading.ClassMirror.DisabledClassMirrorVisitor;
 ...
 * class.
 */
public class DisabledClassDiscovery {
	...
	private synchronized void discover(URL rootLocation) {
		...
		try {
			...
			if (classDiscoveryCache != null) {
				...
				if (debug) {
					StreamUtils.GetSystemOut().println("ClassDiscoveryCache existed for " + rootLocation.toString());
				}
				...
				for (File f : fileList) {
					...
					if (!file.matches(".*\\$(?:\\d)*\\.class") && file.endsWith(".class")) {
						...
						try {
							...
							DisabledClassMirrorVisitor mirrorVisitor = new DisabledClassMirrorVisitor();
							reader.accept(mirrorVisitor,
...
//				}
			...
			if (debug) {
				StreamUtils.GetSystemOut().println(
						"Scans finished for " + rootLocation + ", taking " + (System.currentTimeMillis() - start)
								+ " ms.");
			}
		}
	}
	...
	public List<ClassMirror<?>> getKnownClasses(URL url) {
		...
		doDiscovery();
		if (debug) {
			StreamUtils.GetSystemOut().println("Fetching known classes from " + url.toString());
		}
	...
	public Set<ClassMirror<?>> getClassesWithAnnotation(Class<? extends Annotation> annotation) {
		if (classAnnotationCache.containsKey(annotation)) {
			if (debug) {
				StreamUtils.GetSystemOut().println("Annotation " + annotation.getName() + " was already cached.");
			}
			return new HashSet<>(classAnnotationCache.get(annotation));
		}
		doDiscovery();
		if (debug) {
			StreamUtils.GetSystemOut().println("Fetching classes for uncached annotation " + annotation.getName());
		}
		Set<ClassMirror<?>> mirrors = new HashSet<>();
		for (ClassMirror m : getKnownClasses()) {
			if (debug) {
				if (m.getPackage().getName().startsWith("com.laytonsmith")) {
					StreamUtils.GetSystemOut().println("Checking " + m.getClassName() + " for " + annotation.getName());
				}
			}
			if (m.hasAnnotation(annotation)) {
				if (debug) {
					StreamUtils.GetSystemOut().println(m.getClassName() + " had " + annotation.getName());
				}
				mirrors.add(m);
			}
		}
	...
	public Set<Class> loadClassesWithAnnotation(Class<? extends Annotation> annotation, ClassLoader loader, boolean initialize) {
		...
		for (ClassMirror<?> cm : getClassesWithAnnotation(annotation)) {
			try {
				if (debug) {
					StreamUtils.GetSystemOut().println("Trying to load " + cm.getClassName());
				}
				set.add(cm.loadClass(loader, initialize));
				if (debug) {
					StreamUtils.GetSystemOut().println("Loaded " + cm.getClassName());
				}
			} catch (NoClassDefFoundError e) {
				//Ignore this for now?
				//throw new Error("While trying to process " + cm.toString() + ", an error occurred.", e);
				if (debug) {
					StreamUtils.GetSystemOut().println("Could not find " + cm.getClassName());
				}
			}
		}
		return set;
	...
	/**
	 * Hijacked from ClassDiscovery for testing purposes
	 * <p>
	...
	public static URL GetClassContainer(Class c) {
		...
		if (c.getProtectionDomain().getCodeSource() == null) {
			...
			//so just return null.
			StreamUtils.GetSystemOut().println("No physical container for " + c.getName());
			return null;
		}
		...
		StreamUtils.GetSystemOut().println("Resource: " + thisClass);
		StreamUtils.GetSystemOut().println(
				"Alternatively, " + c.getResource('/' + c.getName().replace('.', '/') + ".class").toString());
		try {
			try {
				packageRoot = StringUtils.replaceLast(thisClass,
						Pattern.quote(c.getName().replaceAll("\\.", "/") + ".class"), ""
				);
			} catch (Exception e) {
				//Hmm, ok, try this then
				packageRoot = c.getProtectionDomain().getCodeSource().getLocation().toString();
				StreamUtils.GetSystemOut().println("Had to use protection domain method...");
			}
			...
			StreamUtils.GetSystemOut().println("Final root: " + packageRoot);
			...
			throw new RuntimeException(
					"While interrogating " + c.getName() + ", an unexpected exception was thrown for potential URL: \""
							+ packageRoot + "\"", e);
		}
	}
}